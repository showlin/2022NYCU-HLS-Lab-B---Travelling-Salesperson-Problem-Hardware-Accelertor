...
void tsp(hls::stream<uint16_t>& streamDistances, unsigned int& shortestDistance)
{
	...
	*/
    ...
    unsigned int v0[N_implement_times>>1][N];
    unsigned int v1[N_implement_times>>1][N];
    //unsigned int v[N_implement_times][N][2];

    loop_inialization: for(int i=0; i<N_implement_times>>1 ;i++){
		for(int j=0;j<N;j++){
			#pragma HLS UNROLL
			v0[i][j] = 100000000;
			v1[i][j] = 100000000;
...
*/
    		...
    		unsigned int t=100000000;
    		    		...
    		    		loop_fill_dsp_block3:    for(int k=0;k<N;k++){
    		    			...
    		    			if( is_odd(i) ){
    		    				t = std::min(t,v0[(~(~i | 1<<j))>>1][k] + int(distances[j][k]) ) ;
    		    			}
    		    			else{
    		    				t = std::min( t,v1[(~(~i | 1<<j))>>1][k] + int(distances[j][k]) ) ;
    		    			}
    		    		}
    ...
    loop_select_candidate:for(int i=1;i<N;i++){
    	...
    	if((N%2)==1)
    		candidate = std::min(candidate,v1[((1<<N)-1)>>1][i]+  int(distances[0][i]) );
    	else
    		candidate = std::min(candidate,v0[((1<<N)-1)>>1][i]+  int(distances[0][i]) );
    }
